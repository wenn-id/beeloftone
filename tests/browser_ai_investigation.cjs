...
module.exports=async({page,login,admin,operator,viewer,apiGet,work})=>{
  ...
  await page.setViewportSize({width:1440,height:1000});
  await page.keyboard.press('Escape');
  console.log('AI investigation browser QA PASS: local analysis, proposal retry, viewer guard, admin approval, atomic production execution, escaping, keyboard, mobile/200%.');
};
