...
module.exports=async({page,login,admin,operator,viewer,apiGet,work})=>{
  ...
  const po=await post('/api/purchase-orders',{reference:'PO-RECEIPT',request_id:pr.id,expected_revision:pr.revision,supplier_id:supplier.id,expected_date:'2026-12-02',terms:'CONTOH tunai',reason:'CONTOH pembelian',prices:[{material_id:m.id,unit_price:'10'}]},'receipt-po');
