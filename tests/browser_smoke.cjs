...
(async () => {
  ...
  assert.equal(await page.getByRole('button',{name:'Ubah tenggat / PIC',exact:true}).count(),0);
  assert.equal(await page.getByRole('button',{name:'Koreksi',exact:true}).count(),0);
  await page.getByRole('button',{name:'Aktivitas harian',exact:true}).click();
  await page.locator('#activity-list .activity-item').first().waitFor();
  const reportDay = await page.getByLabel('Tanggal aktivitas').inputValue();
  assert.ok(reportDay);
  ...
  await page.getByLabel('Tanggal aktivitas').fill('2000-01-01');
  await page.getByRole('button',{name:'Tampilkan aktivitas',exact:true}).click();
  await page.getByText('Tidak ada aktivitas yang cocok pada tanggal ini.',{exact:true}).waitFor();
  assert.equal(await page.locator('#activity-list .activity-item').count(),0);
  await page.getByLabel('Tanggal aktivitas').fill(reportDay);
  await page.getByRole('button',{name:'Tampilkan aktivitas',exact:true}).click();
  await page.locator('#activity-list .activity-item').first().waitFor();
