...
(async () => {
  ...
  async function login(key,{resetViewport=true}={}) {
    if(resetViewport)await page.setViewportSize({width:1440,height:1000});
  ...
  async function openSidebarDestination(name) {
    const destination=page.getByRole('button',{name,exact:true});
    if(!await destination.isVisible()){
      const menu=page.getByRole('button',{name:'Menu',exact:true});
      assert.equal(await menu.isVisible(),true,`${name} requires the mobile Menu control`);
  ...
  const reportDay = await page.locator('#activity-form').getByLabel('Dari tanggal').inputValue();
  assert.ok(reportDay);
  await page.locator('#activity-form').getByLabel('Jenis aktivitas').selectOption('order_changed');
  ...
  // Milestone D: label filter ai-view ("Data sampai tanggal") adalah superstring dari
  // label activity ini, dan getByLabel defaultnya mencocokkan substring pada seluruh
  // DOM, termasuk section yang tersembunyi. Batasi pada form activity saja.
  await page.locator('#activity-form').getByLabel('Dari tanggal').fill('2000-01-01');
  await page.locator('#activity-form').getByLabel('Sampai tanggal').fill('2000-01-01');
  ...
  await page.locator('#activity-form').getByLabel('Dari tanggal').fill(reportDay);
  await page.locator('#activity-form').getByLabel('Sampai tanggal').fill(reportDay);
  await page.getByRole('button',{name:'Tampilkan aktivitas',exact:true}).click();
  await page.locator('#activity-list .activity-item').first().waitFor();
  ...
  await runModule('./browser_materials.cjs');
  await runModule('./browser_bom.cjs');
  await runModule('./browser_reservations.cjs');
  await runModule('./browser_consumption.cjs');
  await runModule('./browser_purchase_requests.cjs');
  await runModule('./browser_purchase_orders.cjs');
  await runModule('./browser_po_receipts.cjs');
  await runModule('./browser_incoming_qc.cjs');
  await runModule('./browser_supplier_returns.cjs');
  await runModule('./browser_supplier_performance_insights.cjs');
  await runModule('./browser_material_price_insights.cjs');
  ...
  const productionCost=await runModule('./browser_production_cost.cjs');
  await runModule('./browser_contribution_margin.cjs',productionCost);
  await runModule('./browser_demand_forecast.cjs');
  await runModule('./browser_replenishment_recommendations.cjs',productionCost);
  await runModule('./browser_integrations.cjs');
  await runModule('./browser_product_external_mappings.cjs');
  await runModule('./browser_jubelio_stock_snapshots.cjs');
  ...
  await runModule('./browser_mekari_payroll_snapshots.cjs');
  await runModule('./browser_payroll_approvals.cjs');
  await runModule('./browser_payroll_payment_reconciliation.cjs');
  ...
  await runModule('./browser_warehouse_movements.cjs',finishedGoods);
  const marketplace=await runModule('./browser_marketplace_reservations.cjs',finishedGoods);
  const picking=await runModule('./browser_marketplace_picking.cjs',{...finishedGoods,...marketplace});
  const packing=await runModule('./browser_marketplace_packing.cjs',{...finishedGoods,...marketplace,...picking});
  const shipping=await runModule('./browser_marketplace_shipping.cjs',{...finishedGoods,...marketplace,...picking,...packing});
  ...
  await runModule('./browser_dialog_closing.cjs');
  ...
  await runModule('./browser_navigation_liquid_lens.cjs');
