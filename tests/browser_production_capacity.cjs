...
module.exports=async({page,login,admin,viewer,apiGet,apiPost,work})=>{
  ...
  const centerMaster=dialog.locator('.product-item').filter({hasText:center.code});
  ...
  await dialog.getByRole('button',{name:'Atur standar waktu',exact:true}).click();
  ...
  assert.equal(await dialog.getByRole('button',{name:'Tambah work center',exact:true}).count(),0);
  ...
  await dialog.getByText('Tidak ada work center yang cocok dengan status dan filter ini.',{exact:true}).waitFor();
