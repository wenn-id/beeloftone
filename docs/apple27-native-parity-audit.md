...
| A4 | **COMPLETE / PR REVIEW**: bounded translucent material on functional chrome only — masthead, sidebar, the sticky approval plate's tint and the one A2/A3 lens — as a CSS-only progressive enhancement over the unchanged A1 solid baseline. One `@supports` gate naming both the standard and `-webkit` property; nested reduced-transparency withdrawal; an explicit `forced-colors` contract; solid automatically wherever the gate fails. No JavaScript changed at all, so the A3 spring, morph and idle-frame behaviour are untouched by construction. Blur measured as real (0.0069 vs 0.2479 luminance spread across a stripe pattern) and bounded (every masthead and selected label ≥4.5:1 on rendered pixels); frame pacing identical to a no-glass control. 631 Python tests and full browser/build validation pass at 0.102.0. See [A4 contract](apple27-functional-glass.md). |
| A5 | **COMPLETE / DRAFT REVIEW**: Command Center golden conversion at 0.103.0, schema 55. Marketplace KPIs and sales hero, real exception pulse, actionable priority queue, aggregate approvals, connector health, and curated business context. Static page-scoped depth behind opaque content; A3 physics and A4 glass containment unchanged. Current rendered before/after contrast passes; zero idle RAF; light/dark, eight widths and 320px at 200% text reviewed. All 637 Python tests, all 84 browser modules, build and package checks pass. OpenAPI differs only by version. Merge, deployment and A6 remain separate. See [A5 report](apple27-command-center-golden.md). |
| A5.1 | **COMPLETE / MERGED #95**: deterministic spring velocity-continuity proof, included in baseline `0f50ceed6c1b7b3f13146264c8d895598e3c9df7`. |
| A5.2 | **HUMAN VISUAL APPROVED 2026-09-24**: operational KPI remap, four equal lower cards, luminous glass materials, and 17-second CSS rim; version 0.104.0, schema 55. See [the final polish evidence](apple27-a52-final-visual-polish.md). PR review and merge remain separate; A6 has not begun. |
| REVIEW GATE | Human review of A5.2 reference parity, behavior, keyboard/assistive behavior, dark/light, 320px/200%, reduced motion/transparency and performance is complete. A6 remains separate work. |
