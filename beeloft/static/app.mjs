...
async function marketplaceShipmentDialog(shipmentId) {
  ...
  try{
    ...
    if(version!==epoch||modal!==dialogVersion||!$('dialog').open)return;
    $('dialog-content').innerHTML=`<p class="form-info">${e(row.reference)} · ${row.status==='shipped'?'Sudah dikirim':'Sudah dikoreksi'}</p><h3>${n(row.quantity)} pcs · ${e(row.sku)} · ${e(row.size)}</h3><article class="material-event"><h3>${e(row.marketplace)} · ${e(row.external_order_reference)}</h3><p>${e(row.carrier)} · resi ${e(row.tracking_number)}</p><p>Dikirim ${date(row.shipped_date)} dari ${e(row.staging_location)}</p><p>Diretur ${n(row.returned_quantity)} pcs · sisa dapat diretur ${n(row.returnable_quantity)} pcs</p><p>Sumber pack ${e(row.pack_reference)} · ${date(row.packed_date)}</p></article><p>Pick ${e(row.pick_reference)} · reservasi ${e(row.reservation_reference)} · penerimaan ${e(row.receipt_reference)} · Final QC ${e(row.final_qc_reference)} · batch ${e(row.batch_reference)}</p><p class="reason">${e(row.reason)}</p><p class="hint">${e(row.actor_name)} · ${purchaseStamp(row.created_at)}</p>${row.returned_quantity?'<p class="hint">Koreksi semua retur aktif sebelum mengoreksi pengiriman.</p>':''}${row.reversal?`<article class="material-event"><h3>Pengiriman dikoreksi</h3><p class="reason">${e(row.reversal.reason)}</p><p class="hint">${e(row.reversal.actor_name)} · ${purchaseStamp(row.reversal.created_at)}</p></article>`:''}<div class="actions"><button data-action="marketplace-pack" data-id="${e(row.pack_id)}">Pack asal</button><button data-action="marketplace-shipments" data-id="${e(row.order_id)}">Semua pengiriman</button><button data-action="marketplace-returns" data-id="${e(row.order_id)}">Semua retur</button><button data-action="warehouse" data-id="${e(row.order_id)}">Inventori gudang</button>${user.role!=='viewer'&&row.status==='shipped'&&row.returnable_quantity>0?`<button data-action="new-marketplace-return" data-id="${e(row.id)}">Catat retur</button>`:''}${user.role==='admin'&&row.status==='shipped'&&!row.returned_quantity?'<button id="reverse-marketplace-shipment">Koreksi pengiriman</button>':''}</div>`;
