...
async function reworkCompletionForm(qcId) {
  ...
  try{
    ...
    if(version!==epoch || modal!==dialogVersion || !$('dialog').open)return;
    if(qc.status!=='completed' || qc.rework_completable_quantity<1){$('dialog-content').innerHTML=`<p>${qc.untraced_rework_return_quantity?`Rework catatan ini sudah dikembalikan ke QC oleh ${n(qc.untraced_rework_return_quantity)} pcs perpindahan tanpa catatan selesai rework. Koreksi perpindahan tersebut dari riwayat perpindahan order lebih dahulu.`:'Final QC sudah dikoreksi atau seluruh rework sudah diselesaikan. Muat ulang rinciannya.'}</p><button data-action="final-qc-record" data-id="${e(qcId)}">Muat ulang final QC</button>`;return;}
